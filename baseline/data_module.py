...
class TextForgetDatasetQA(Dataset):
    def __init__(self, data_path, tokenizer, model_family,  max_length=512, split = "forget10", loss_type="idk"):
        ...
        if "forget05_seq" in split:
            retain_split = split.replace("forget05", "retain")
        else:
            retain_split = "retain" + str(100 - int(split.replace("forget", ""))).zfill(2)
        # retain_split = "retain_perturbed"
        # import pdb ; pdb.set_trace()
...
class WMDPDataset:
    def __init__(self, seed=42, ratio=1.0, subset="forget", min_len = 0):
        self.ratio = ratio
        self.seed = seed
        self.subset = subset
        self.dataset = self.get_dataset(min_len)

    def get_dataset(self, min_len=0):
        
        if self.subset == "retain":
            train_dataset_cyber = load_dataset(
                "cais/wmdp-corpora", "cyber-retain-corpus", cache_dir="/egr/research-dselab/renjie3/.cache"
            )["train"]
            # train_dataset_cyber = load_dataset("wikitext", "wikitext-2-raw-v1", split="test")
            # train_dataset_cyber = train_dataset_cyber.filter(lambda x: len(x['text']) > min_len)
            
            train_dataset_bio = load_dataset(
                "cais/wmdp-corpora", "bio-retain-corpus", cache_dir="/egr/research-dselab/renjie3/.cache"
            )["train"]
            # train_dataset_bio = load_dataset("wikitext", "wikitext-2-raw-v1", split="test")
            # train_dataset_bio = train_dataset_bio.filter(lambda x: len(x['text']) > min_len)
            ...
            train_dataset_bio = load_dataset(
                "json",
                data_files="wmdp_data/bio-forget-corpus.jsonl",
                split="train",
                cache_dir="./.cache",
            )
